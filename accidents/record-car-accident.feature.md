# Record Car Accident

## Customer calls accident hotline and leaves a voice message v1 [published]

Customer calls accident hotline and leaves a message with accident info.

## Customer calls accident hotline and talks to a live agent v1 [published]

Customer calls accident hotline, talks to the agent and leaves basic accident info.

## Customer decides not to report accident v1 [published]

Customer may decide not to report

## Customer provides passenger info v1 [published]

User enters {first_name} {last_name}, {relationship}, and {injured Y/N}

| first_name | last_name | relationship |
| ---------- | --------- | ------------ |
| Jay        | Ubuntu    | Uncle        |

### Tests

| Test |
| --- |
| https://github.com/vydra/GatlingGen/blob/main/app/src/test/java/ggen/cli/CLITest.java |
| https://github.com/vydra/GatlingGen/blob/main/app/src/test/java/ggen/cli/SecondTest.java |
